...
if (!class_exists('ElfsightFacebookFeedPlugin')) {
    class ElfsightFacebookFeedPlugin {
        private $name;
        private $slug;
        private $version;
        private $textDomain;
        ...
        public function __construct($config) {
            ...
            $this->update = new ElfsightFacebookFeedPluginUpdate($this->updateUrl, $this->version, $this->pluginSlug, $this->purchaseCode);
            $this->widgetsApi = new ElfsightFacebookFeedWidgetsApi($this->slug, $this->pluginFile, $this->textDomain);
            $this->admin = new ElfsightFacebookFeedPluginAdmin($config, $this->widgetsApi);
            $this->widget = new ElfsightFacebookFeedWidget($config, $this->widgetsApi);
            $this->vcElement = new ElfsightFacebookFeedVCElement($config, $this->widgetsApi);
        ...
        public function recursiveDefaults($properties, $defaults){
            foreach($properties as $property) {
                ...
                } else if (isset($property['id']) && isset($property['defaultValue'])) {
        ...
        public function addShortcode($atts) {
            ...
            $result = '<div class="elfsight-widget-' . str_replace('elfsight-', '', $this->slug) . ' elfsight-widget" data-' . $this->slug . '-options="' . $options_string . '" data-' . $this->slug . '-version="' . $version . '"></div>';
